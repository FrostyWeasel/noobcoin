...
class State:
    ...
    def from_dictionary(cls, dictionary):
        ...
        processed_transactions = dictionary['processed_transactions']
        
        return cls(utxos, processed_transactions)
    
    @staticmethod
    def utxos_to_dict(utxos):
        utxos_copy = deepcopy(utxos)
        
        for key, utxo_dict in utxos.item():
            utxos_copy[key] = {utxo_id: utxo.to_dict() for utxo_id, utxo in utxo_dict}
    ...
    def utxos_from_dict(utxos: dict[str, dict[str, dict]]) -> dict[str, dict[str, TransactionOutput]]:
        for key, utxo_dict in utxos.item():
            utxos[key] = {utxo_id: TransactionOutput.from_dictionary(utxo) for utxo_id, utxo in utxo_dict}
