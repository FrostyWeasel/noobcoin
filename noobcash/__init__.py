...
def create_node():
    ...
    if am_bootstrap is True:        
        ...
        while True:
            try:
                r = requests.post(f"http://{me.ring[0]['ip']}:{me.ring[0]['port']}/contact", data={'node_public_key': me.wallet.public_key.decode("utf-8"), 'node_ip_address': my_ip_address, 'node_port': port_number})
                break
            except:
                pass
